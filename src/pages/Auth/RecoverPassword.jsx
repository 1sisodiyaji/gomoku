  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="keywords" content="Recover password , forgot password , forgot password in Gomoku" />
        ...
        <title>Recover Password | Gomoku</title>
        <meta property="og:title" content=" Recover Password| | Gomoku" />
        ...
        <meta property="og:url" content="https://Gomoku.com/RecoverPassword" />
        <meta property="og:type" content="Education-Website" />
        <link rel="icon" type="image/png" href="https://Gomoku.com/img/favicon.ico" sizes="32x32" />
      </Helmet>
          ...
          <form>
            ...
            {showEmails && (
              <>
                ...
                >
                  {loading ? <>  
                    Sending... <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span></> : 'Send OTP'}
                </button>
              </>
            )}
            {showOtp && (
                ...
                <div className="row">
                  ...
                  <div>
                    <button type="button" onClick={verifyOtp} className="btn btn-block my-4 text-capitalize py-3" style={{ fontSize: "1rem" }}>
                      {loading ? <> Verifying Otp... <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> </> : 'Verify OTP'}
                    </button>
                  </div>
                </div>
              </div>
            ...
            {showPassword && (
              <div style={{ display: "block" }} id="passwordDisplay">
                <div className="input-group mb-4 mt-2">
                  ...
                  />
                  <i onClick={passwordView} className="fi fi-ss-eye-crossed pt-2 ps-2" style={{cursor: 'pointer'}}></i>
                </div>
                <div className="mb-4 input-group">
                  ...
                  />
                  <i onClick={passwordView2} className="fi fi-ss-eye-crossed pt-2 ps-2" style={{cursor: 'pointer'}}></i>
                </div>
                {/* Update Password button */}
                <button type="button" onClick={updatePassword} className="btn btn-block mb-4 text-capitalize py-3" style={{ fontSize: "1rem" }}>
                  {loading ? <> Updating Password  <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> </> : 'Update'}
                </button>
              </div>
            )}
          </form>
